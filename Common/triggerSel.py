...
class Triggers():
    ...
    def ApplyTriggers(self, df, offline_lepton_legs, isData = False, isSignal=False, channel = "bbtautau", default=0):
        hltBranches = []
        ...
        for path, path_dict in self.trigger_dict.items():
            ...
            for leg_id, leg_tuple in enumerate(path_dict['legs']):
                ...
                if not leg_tuple["doMatching"]:
                    ...
                    if channel == "bbtautau":
                        ...
                        df = df.Define(f'{type_name_offline}_{var_name_offline}_sel', f'HwwCandidate.isLeg({type_name_offline}_idx, {self.dict_legtypes[type_name_offline]}) && ({var_name_offline})')
            ...
            for offline_leg_id, offline_leg_name in enumerate(offline_lepton_legs):
                ...
                #df = df.Define(canidate_leg_size,"HwwCandidate.leg_type.size() > {offline_leg_id}")
                if channel == "bbww":
                    cond = f"HwwCandidate.leg_type.size() > {offline_leg_id}"
                    df = df.Define(matching_var_bool,
                                f'{cond} ? (hasOOMatching_{path}_details.second.count(LegIndexPair(HwwCandidate.leg_type.at({offline_leg_id}), HwwCandidate.leg_index.at({offline_leg_id}) ) ) > 0 ) : 0')
                else:
                    df = df.Define(matching_var_bool,
